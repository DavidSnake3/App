...
export function DonaGastosWidget({ size }: { size: WidgetSize }) {
  ...
  if (!s || total <= 0) {
    return (
      <div className="card p-4 h-full flex flex-col items-center justify-center text-center">
      ...
      <div className={`flex items-center gap-4 mt-2 flex-1 ${compact ? 'flex-col !gap-2' : ''}`}>
        <div className="relative shrink-0" style={{ width: compact ? 92 : 118, height: compact ? 92 : 118 }}>
          <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
            ...
            {segs.map((p) => (
              <circle
                ...
                stroke={p.color} strokeWidth="14" strokeLinecap="butt"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="num text-[13px] font-bold text-ink leading-none">{formatMoneyShort(total)}</span>
          ...
          return (
            <div key={d.id} className="flex-1 flex flex-col items-center gap-1 min-w-0">
              ...
              <div className="flex items-end gap-[3px]" style={{ height: H }}>
                <div
                  className="w-[13px] rounded-t-md"
                  style={{ height: Math.max(3, (d.income / max) * H), background: 'var(--c-income)', opacity: 0.9 }}
                  title={`Ingresos: ${formatMoney(d.income)}`}
                />
                <div
                  className="w-[13px] rounded-t-md"
                  style={{ height: Math.max(3, (d.expenses / max) * H), background: 'var(--c-danger)', opacity: 0.9 }}
